...
> | 21 | `closed_ended_dialog.dart` — เมื่อเปิดแป้นพิมพ์ เนื้อหาใน dialog "คำถามปลายปิด" (ช่องตัวเลือก/ปุ่มยืนยัน) ถูกบังและมองไม่เห็น | `Dialog` ของ Flutter คำนวณ `MediaQuery.viewInsetsOf(context) + insetPadding` และห่อ child ด้วย `AnimatedPadding` + `MediaQuery.removeViewInsets` อยู่แล้ว แต่โค้ด dialog เติม `Padding(bottom: viewInsets.bottom)` จาก `MediaQuery.of(context).viewInsets` ซ้ำอีกชั้น → viewport ของ `SingleChildScrollView` เหลือ `จอ − 2×แป้นพิมพ์ − inset` เนื้อหาจึงถูกบีบและถูกบัง | ลบ `Padding(bottom: viewInsets)` และย้าย padding ของเนื้อหาไปที่ `SingleChildScrollView(padding: EdgeInsets.all(20))` เพื่อพึ่งกลไก keyboard inset ของ `Dialog` เพียงครั้งเดียว; เพิ่ม widget regression test จำลอง `viewInsets.bottom = 300` แล้วตรวจว่า scroll view เต็มพื้นที่ dialog และขอบล่างไม่ต่ำกว่าแป้นพิมพ์ |
> | 22 | `chart_board_page.dart` + `chat_repository.dart` + `chat-api.js` + migration — ฝั่งผู้ป่วยสลับไปตอบคำถามบังคับข้ออื่นแล้วปุ่มของข้อเดิมยังเป็นสีเหลือง (กำลังตอบ) ทำให้ผู้เชี่ยวชาญเข้าใจผิดว่าผู้ป่วยตอบหลายข้อพร้อมกัน | ตอนแตะคำถามมีแค่การเปลี่ยนสถานะของข้อที่แตะเป็น `reading` แต่ไม่มีใครย้อนสถานะข้อเดิมกลับเป็น `unread`; ทั้ง `markClosedEndedQuestionReading` และ `_onClosedEndedClose` ถูกออกแบบให้ "ปิด/สลับแล้วคง `reading`" (Phase 6.14 เดิม) จึงค้างเป็นสีทองหลายปุ่ม | เพิ่ม `RequiredQuestionStatus.staleReadingQuestions` (pure helper) + `_revertReadingRequiredQuestions({keepId})` ให้ย้อนทุกข้อที่เป็น `reading` กลับเป็น `unread` ก่อนเปิดข้อใหม่ และเมื่อปิด overlay; closed-ended ย้อนผ่าน RPC ใหม่ `mark_closed_ended_question_unread_backend` + route `POST /api/chat/closed-ended/:id/unread` + `ChatRepository.markClosedEndedQuestionUnread` (เพราะ guard trigger ห้าม client update `required_status` ตรงๆ); open-ended ใช้ `updateRequiredStatus(unread)` เดิม; อัปเดต local optimistic state ให้สีเปลี่ยนทันที; เพิ่ม unit tests ใน `required_question_status_test.dart`. **หมายเหตุ:** เปลี่ยน semantics เดิมที่ "สลับข้อแล้วคง reading" — ตอนนี้มีข้อที่เป็น `reading` ได้ครั้งละ 1 ข้อ |
...
- **มีข้อที่เป็น `reading` (ทอง) ได้ครั้งละ 1 ข้อ** — เมื่อผู้ป่วยสลับไปตอบข้ออื่น หรือปิด overlay คำถามข้อเดิมต้องกลับเป็น `unread` (แดง) ทันที ไม่เช่นนั้นผู้เชี่ยวชาญจะเห็นจุด "กำลังพิมพ์" หลายปุ่มพร้อมกันและเข้าใจผิดว่าผู้ป่วยตอบหลายข้อ (ดู bug #22)
...
- [x] แตะพื้นที่ว่างใน dialog เพื่อซ่อนแป้นพิมพ์ (`GestureDetector` + `FocusScope.of(context).unfocus()`); TextField/ปุ่มยังจัดการแตะของตัวเอง
