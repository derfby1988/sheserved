...
class TlzDrawer extends StatefulWidget {
  ...
  const TlzDrawer({super.key, this.onClose, this.onLogout});

  @override
  State<TlzDrawer> createState() => _TlzDrawerState();
}

class _TlzDrawerState extends State<TlzDrawer>
  ...
  void _checkVolunteerRole() async {
    ...
    if (user != null) {
      final isVol = await ServiceLocator.instance.videoRepository
          .isUserVolunteer(user.id);
      if (mounted) {
  ...
  void _checkDrugRiskPermission() async {
    ...
    if (user.professionId != null) {
      try {
        ...
        if (mounted) {
          setState(
            () => _canManageDrugRisk = profession?.canManageDrugRisk ?? false,
          );
        }
      } catch (e) {
  ...
  void _handleSwipeUpdate(DragUpdateDetails details) {
    ...
    final progressDelta =
        -delta / screenWidth; // เปลี่ยนเป็นบวกเมื่อปัดไปทางซ้าย
    final currentProgress = _animationController.value;
  ...
  Widget build(BuildContext context) {
    ...
    final drawerWidth = math.min(
      screenWidth * 0.75,
      320.0,
    ); // ใช้ 75% แต่ไม่เกิน 320px เพื่อความสวยงาม
    ...
    _animationController.forward().then((_) {
      ...
      if (_isProtectedRoute(route) && !AuthService.instance.isLoggedIn) {
        Navigator.pushNamed(
          context,
          '/login',
          arguments: {'redirect': route, 'args': arguments},
        );
        return;
      }

      // Navigate to the route
      Navigator.pushNamed(context, route, arguments: arguments).catchError((
        error,
      ) {
