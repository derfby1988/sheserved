...
class _PlatformSettingsPageState extends State<PlatformSettingsPage> {
  ...
  bool _googlePlacesFallback = true;
  ...
  void initState() {
    ...
    _googlePlacesFallback = PlatformService.isGooglePlacesFallbackEnabled;
    _loadMetrics();
  }
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    _buildPlatformCard(
                      ...
                      description:
                          'แนะนำให้ปิดบน Web เพื่อลดค่าใช้จ่าย Google Maps JS API และเพิ่มความเร็วในการโหลดหน้าแรก',
                      child: _webLiveMap
                          ? Column(
                              children: [
                                ...
                                _buildSubToggle(
                                  ...
                                  value: PlatformService.shouldShowLiveMap(
                                    pageName: 'home',
                                  ),
                                  onChanged: (v) => setState(
                                    () => PlatformService.updateWebMapSetting(
                                      'home',
                                      v,
                                    ),
                                  ),
                                ...
                                _buildSubToggle(
                                  title:
                                      'หน้าสร้างก๊วน (Sport Group Create Map)',
                                  value: PlatformService.shouldShowLiveMap(
                    ...
                    _buildPlatformCard(
                      ...
                      description:
                          'ใช้งาน Google Maps SDK สำหรับ iOS (ฟรีตามโควต้า Native)',
                    ),
                    const SizedBox(height: 16),
                    _buildPlatformCard(
                      ...
                      description:
                          'ใช้งาน Google Maps SDK สำหรับ Android (ฟรีตามโควต้า Native)',
                    ),
                    const SizedBox(height: 16),
                    _buildPlatformCard(
                      title: 'Google Places Search Fallback',
                      icon: Icons.search,
                      color: Colors.amber.shade800,
                      isEnabled: _googlePlacesFallback,
                      onChanged: (v) {
                        setState(() => _googlePlacesFallback = v);
                        PlatformService.setGooglePlacesFallbackEnabled(v);
                      },
                      description:
          ...
          if (_isLoadingMetrics)
            ...
            Column(
              children: _metrics.map((m) {
                final platform =
                    m['platform']?.toString().toUpperCase() ?? 'UNKNOWN';
                ...
                final pageName = metricName
                    .replaceAll('map_load_', '')
                    .toUpperCase();
                final displayLabel = pageName == 'TOTALS_TOTAL'
                    ? 'TOTAL ALL'
                    : '$platform ($pageName)';
    ...
    return ElevatedButton(
      onPressed: () {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('บันทึกการตั้งค่าแพลตฟอร์มสำเร็จ'),
