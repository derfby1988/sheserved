...
class _DonationDashboardPageState extends State<DonationDashboardPage> {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (args == 'auto_create') {
        shouldAutoCreate = true;
          ...
          (args['action'] == 'auto_create' ||
              (args['args'] is Map &&
                  args['args']['action'] == 'auto_create'))) {
        shouldAutoCreate = true;
      }
  ...
  Future<void> _loadData() async {
    ...
    if (currentUser != null) {
      try {
        final refunds = await _repository.getRefundPendingTransactions(
          currentUser.id,
        );
        if (mounted) {
          setState(() {
    ...
    return Scaffold(
      ...
      floatingActionButton: Padding(
        padding: const EdgeInsets.only(
          bottom: 80.0,
        ), // เลื่อนปุ่มให้สูงขึ้นจาก BottomNavigationBar ขึ้นไปอีก
        child: SizedBox(
          ...
          child: FloatingActionButton.extended(
            ...
            label: const Text(
              ...
              style: TextStyle(
                ...
                fontSize:
                    13, // ลดขนาดฟอนต์ลงนิดหน่อยเพื่อให้สมดุลกับความสูง 35px
              ),
            ),
          ),
        ),
      ...
      body: StreamBuilder<List<DonationCategory>>(
        ...
        builder: (context, catSnapshot) {
          if (catSnapshot.hasError) {
            return Center(
              child: Text(
                'เกิดข้อผิดพลาดในการโหลดหมวดหมู่: ${catSnapshot.error}',
              ),
            );
          }
          ...
          final emergencyCategories = categories
              .where((c) => c.isEmergency)
              .toList();
          final generalCategories = categories
              .where((c) => !c.isEmergency)
              .toList();
          ...
          return StreamBuilder<List<DonationRequest>>(
            ...
            builder: (context, reqSnapshot) {
              if (reqSnapshot.hasError) {
                return Center(
                  child: Text(
                    'เกิดข้อผิดพลาดในการโหลดรายการ: ${reqSnapshot.error}',
                  ),
                );
              }
              final trendingRequests =
                  reqSnapshot.data?.where((r) => r.isTrending).toList() ?? [];
                    ...
                    Stack(
                      children: [
                        ...
                        Container(
                          ...
                          decoration: const BoxDecoration(
                            image: DecorationImage(
                              image: AssetImage(
                                "assets/images/donation_banner.png",
                              ),
                              fit: BoxFit.cover,
                            ),
                          ),
                        ...
                        Padding(
                          padding: const EdgeInsets.only(
                            top: 190,
                          ), // Offset down to overlap bottom of banner
                          child: Container(
                            width: double.infinity,
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          // ใช้ Row + Expanded แทน GridView เพื่อให้ความสูงช่อง
                          // ยืดตามเนื้อหา (ไอคอน+เลเบล 2 บรรทัด) ไม่ล้นด้านล่าง
                          Column(
                            children: [
                              for (
                                var i = 0;
                                i < generalCategories.length;
                                i += 4
                              ) ...[
                                Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    for (var j = i; j < i + 4; j++)
                                      Expanded(
                                        child: j < generalCategories.length
                                            ? Padding(
                                                padding:
                                ...
                                if (i + 4 < generalCategories.length)
                                  const SizedBox(height: 16),
                              ],
                            ],
                          ),
                        ...
                        children: [
                          ...
                          SizedBox(
                            ...
                            child: trendingRequests.isEmpty
                                ? const Center(
                                    child: Text('ไม่มีข้อมูลยอดนิยมในขณะนี้'),
                                  )
                                : ListView.builder(
                                    scrollDirection: Axis.horizontal,
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setDialogState) => AlertDialog(
          title: const Text(
            'จัดการเงินคืน (Refund Resolution)',
            style: TextStyle(fontWeight: FontWeight.bold),
          ),
          content: SizedBox(
            width: 400,
                            ...
                            children: [
                              Expanded(
                                child: OutlinedButton(
                                  onPressed: () async {
                                    await _repository.updateRefundPreference(
                                      tx['id'],
                                      'credit',
                                    );
                                    _loadData(); // Re-check
...
class _PulseProgressBarState extends State<_PulseProgressBar>
          ...
          children: [
            FractionallySizedBox(
              ...
              child: Container(
                decoration: BoxDecoration(
                  gradient: LinearGradient(
                    colors: [Colors.blueAccent, Colors.cyanAccent.shade400],
                  ),
                ),
              ),
            ),
            AnimatedBuilder(
              ...
              builder: (context, child) {
                return Positioned(
                  left:
                      (widget.progress * MediaQuery.of(context).size.width) *
                      _animation.value,
                  child: Container(
                    width: 40,
                    height: 6,
        ...
        children: [
          ...
          Row(
            ...
            children: List.generate(
              4,
              (i) => CircleAvatar(radius: 30, backgroundColor: Colors.white),
            ),
          ),
