...
class EditGroupSheet {
  /// After creating a group: if the position feature is on but positions
  /// aren't configured yet, prompt the owner to set them up (opens the
  /// edit sheet on the "ตำแหน่ง" tab).
  static Future<void> maybePromptPositionSetup(
    BuildContext pageContext, {
    required FitnessBuddiesRepository repo,
    required SupabaseClient client,
    required List<Map<String, dynamic>> knownGroups,
    ...
    if (group == null || !pageContext.mounted) return;
    ...
    if (shouldSetup != true || !pageContext.mounted) return;
    await EditGroupSheet.show(
      pageContext,
      repo: repo,
      client: client,
      group: group,
