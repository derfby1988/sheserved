...
class HomeHeaderSection extends StatelessWidget {
  ...
  final Function(Map<String, dynamic> alert)? onFitnessBookingAlertTapped;
  ...
  Widget build(BuildContext context) {
    final hasNotificationContent =
        alerts.isNotEmpty ||
        donationAlerts.isNotEmpty ||
        yieldWayAlerts.isNotEmpty ||
        consultationAlerts.isNotEmpty ||
        employeeInvitationAlerts.isNotEmpty ||
        cancelledEmployeeInvitationAlerts.isNotEmpty ||
        fitnessBookingAlerts.isNotEmpty;
      ...
      child: Row(
        ...
        children: [
          ...
          ConstrainedBox(
            ...
            child: isLoading && !hasNotificationContent
                ...
                : _ScrollableNotificationContent(
                    child: Builder(
                      builder: (context) {
                        ...
                        debugPrint(
                          'HomeHeader: employeeInvitationAlerts.length=${employeeInvitationAlerts.length}',
                        );
                        ...
                        debugPrint(
                          'HomeHeader: cancelledEmployeeInvitationAlerts.length=${cancelledEmployeeInvitationAlerts.length}',
                        );

                        // 1. Consultation alerts (สำคัญรองลงมา)
                        debugPrint(
                          'HomeHeader: consultationAlerts.length=${consultationAlerts.length}',
                        );
                        for (var c in consultationAlerts) {
                          combinedItems.add({
                            'time':
                                c['requestedAt'] as DateTime? ?? DateTime.now(),
                            'type': 'consultation',
                            'data': c,
                          });
                        }
                        ...
                        for (var d in donationAlerts) {
                          combinedItems.add({
                            'time':
                                d['updatedAt'] as DateTime? ?? DateTime.now(),
                        ...
                        combinedItems.sort((a, b) {
                          final aPinned =
                              a['type'] == 'employee_invitation' ||
                              a['type'] == 'employee_invitation_cancelled' ||
                              a['type'] == 'consultation';
                          final bPinned =
                              b['type'] == 'employee_invitation' ||
                              ...
                              .map((item) {
                                ...
                                if (item['type'] == 'donation_update') {
                                  ...
                                  final groupName =
                                      ...
                                      b['group_name']?.toString() ??
                                      '';
                                  final requesterName =
                                      b['requesterName']?.toString() ??
                                      b['requester_name']?.toString() ??
                                      '';
                                  final message =
                                      b['message']?.toString() ?? '';
                                  ...
                                  if (groupName.isNotEmpty)
                                    detailParts.add(groupName);
                                  if (requesterName.isNotEmpty)
                                    detailParts.add(requesterName);
                                  return GestureDetector(
                                    onTap: () =>
                                        onFitnessBookingAlertTapped?.call(b),
                                    child: Padding(
                                      padding: const EdgeInsets.symmetric(
                                        vertical: 4,
                                      ),
                                  ...
                                  final professionName =
                                      inv['profession_name']?.toString() ?? '';
                                  final organizationName =
                                      inv['organization_name']?.toString() ??
                                      '';
                                  final displayOrganizationName =
                                      organizationName.isNotEmpty &&
                                      organizationName != professionName;
                                  final jobTitle =
                                      inv['job_title']?.toString() ?? '';
                                  return GestureDetector(
                                    onTap: () =>
                                        onEmployeeInvitationTapped?.call(token),
                                    child: Container(
                                  ...
                                  final cancelledByName =
                                      inv['cancelled_by_name']?.toString() ??
                                      '';
                                  final cancellationReason =
                                      inv['cancellation_reason']?.toString() ??
                                      '';
                                  return Container(
                                    margin: const EdgeInsets.symmetric(
                                      vertical: 3,
                                  ...
                                  return Dismissible(
                                    ...
                                    onDismissed: (_) =>
                                        onConsultationAlertDismissed?.call(
                                          consultId,
                                        ),
                                    ...
                                    child: GestureDetector(
                                      onTap: () => onConsultationAlertTapped
                                          ?.call(consultId),
                                      child: Container(
                                        margin: const EdgeInsets.symmetric(
                                          vertical: 3,
                                  ...
                                  final categoryName =
                                      alert['categoryName'] ?? 'แจ้งเหตุ';
                                  final isVolunteer =
                                      alert['isVolunteer'] == true;

                                  final distanceVal =
                                      alert['distance'] as double? ?? 0.0;
                                  String distanceText = '';
                                  if (distanceVal > 0) {
                                    if (distanceVal < 1000) {
                                      distanceText =
                                          ' ${distanceVal.toStringAsFixed(0)} ม.';
                                    } else {
                                      distanceText =
                                          ' ${(distanceVal / 1000).toStringAsFixed(1)} กม.';
                                    }
                                  }

                                  final text =
                                      (isVolunteer
                                          ? 'ขอจิตอาสาช่วย$categoryName'
                                          : 'เกิด$categoryName') +
                                      distanceText;
                                  final textColor = isVolunteer
                                      ? const Color(0xFFF5A623)
...
class _ScrollableNotificationContent extends StatefulWidget {
  ...
  @override
  State<_ScrollableNotificationContent> createState() =>
      _ScrollableNotificationContentState();
}

class _ScrollableNotificationContentState
    extends State<_ScrollableNotificationContent> {
