-- Fix: RPC functions use wrong room_id when consultation_requests.room_id is null
-- Fallback chain: consultation_requests.room_id → chat_rooms.id → 'consult_' + consultation_id

-- Helper: get_room_id with fallback
...
BEGIN
  ...
  IF v_room_id IS NULL OR v_room_id = '' THEN
    SELECT croom.id INTO v_room_id
    FROM public.chat_rooms croom
    WHERE croom.consultation_id = p_consultation_id
    LIMIT 1;
  END IF;
...
-- 1. Fix get_expert_completion_status
...
BEGIN
  ...
  FROM public.users u WHERE u.id = p_provider_id;

  IF v_package_id IS NULL OR v_profession_id IS NULL THEN
    RETURN json_build_object('can_finish', true, 'progress', 100, 'items', '[]'::JSONB);
  END IF;
  ...
  FROM public.prescriptions WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id;
  SELECT COUNT(*) INTO v_approved_count
  FROM public.prescriptions WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id AND is_approved = true;

  SELECT COUNT(*) INTO v_question_count
  FROM public.chat_messages WHERE room_id = v_room_id AND is_required = true AND required_owner_id = p_provider_id;
  SELECT COUNT(*) INTO v_answered_count
  FROM public.chat_messages WHERE room_id = v_room_id AND is_required = true AND required_owner_id = p_provider_id AND required_status = 'answered';
  SELECT COUNT(*) INTO v_unanswered_count
  FROM public.chat_messages WHERE room_id = v_room_id AND is_required = true AND required_status != 'answered';

  SELECT COALESCE(has_video_call, false), COALESCE(has_assessment, false)
  INTO v_has_video_call, v_has_assessment
  FROM public.consultation_room_experts WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id;

  SELECT COUNT(*) INTO v_general_message_count
  FROM public.chat_messages WHERE room_id = v_room_id AND sender_id = p_provider_id AND (is_required = false OR is_required IS NULL);
  ...
  RETURN json_build_object(
    'can_finish', (NOT v_rules.must_prescribe OR v_prescription_count > 0) AND
    ...
    'prescription_count', v_prescription_count, 'approved_count', v_approved_count,
    'question_count', v_question_count, 'answered_count', v_answered_count,
    'unanswered_count', v_unanswered_count, 'has_video_call', v_has_video_call,
    'has_assessment', v_has_assessment, 'general_message_count', v_general_message_count
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 2. Fix can_expert_finish_job
...
BEGIN
  ...
  IF v_rules.must_prescribe THEN
    SELECT COUNT(*) INTO v_prescription_count
    FROM public.prescriptions WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id;
    IF v_prescription_count = 0 THEN v_missing := v_missing || to_jsonb('ต้องออกใบสั่งยา');
    ELSIF v_rules.min_prescription_items > 0 AND v_prescription_count < v_rules.min_prescription_items THEN
      v_missing := v_missing || to_jsonb('ต้องออกยาขั้นต่ำ ' || v_rules.min_prescription_items || ' รายการ (ปัจจุบัน ' || v_prescription_count || ')');
    END IF;
    IF v_rules.requires_prescription_approval THEN
      SELECT COUNT(*) INTO v_prescription_count
      FROM public.prescriptions WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id AND is_approved = true;
      IF v_prescription_count = 0 THEN v_missing := v_missing || to_jsonb('ใบสั่งยาต้องได้รับการอนุมัติ'); END IF;
    END IF;
  END IF;
  ...
  IF v_rules.min_required_questions > 0 THEN
    ...
    IF v_question_count < v_rules.min_required_questions THEN
      v_missing := v_missing || to_jsonb('ต้องตั้งคำถามบังคับขั้นต่ำ ' || v_rules.min_required_questions || ' ข้อ (ปัจจุบัน ' || v_question_count || ')');
    END IF;
  END IF;
  ...
  IF v_rules.must_answer_all_questions THEN
    ...
    IF v_unanswered_count > 0 THEN
      v_missing := v_missing || to_jsonb('มีคำถามบังคับที่ยังไม่ได้ตอบ ' || v_unanswered_count || ' ข้อ');
    END IF;
  END IF;
  ...
  IF v_rules.min_general_messages > 0 THEN
    ...
    IF v_general_message_count < v_rules.min_general_messages THEN
      v_missing := v_missing || to_jsonb('ต้องส่งข้อความขั้นต่ำ ' || v_rules.min_general_messages || ' ข้อ (ปัจจุบัน ' || v_general_message_count || ')');
    END IF;
  END IF;
  ...
  IF v_rules.requires_video_call THEN
    ...
    FROM public.consultation_room_experts WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id;
    IF NOT v_has_video_call THEN v_missing := v_missing || to_jsonb('ต้องทำ Video Call'); END IF;
  END IF;

  IF v_rules.requires_health_assessment THEN
    SELECT COALESCE(has_assessment, false) INTO v_has_assessment
    FROM public.consultation_room_experts WHERE consultation_id = p_consultation_id AND provider_id = p_provider_id;
    IF NOT v_has_assessment THEN v_missing := v_missing || to_jsonb('ต้องทำ Health Assessment'); END IF;
  END IF;

  RETURN json_build_object('can_finish', jsonb_array_length(v_missing) = 0, 'missing_requirements', v_missing);
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
