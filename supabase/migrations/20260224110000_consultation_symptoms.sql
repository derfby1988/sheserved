...
CREATE INDEX IF NOT EXISTS idx_consultation_symptoms_request_id ON consultation_symptoms(request_id);
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'consultation_symptoms' AND policyname = 'Users can view symptoms of their own requests'
    ) THEN
...
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM pg_policies WHERE tablename = 'consultation_symptoms' AND policyname = 'Users can insert symptoms for their own requests'
    ) THEN
